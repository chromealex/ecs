...
namespace Pathfinding {
	...
	public struct GraphMask {
		...
		public GraphMask (long value) {
			this.value = value;
		}

		public static implicit operator long(GraphMask mask) {
			return mask.value;
		}

		public static implicit operator GraphMask (long mask) {
		...
		public static GraphMask FromGraph (NavGraph graph) {
			...
			return (long)(1 << (int)graph.graphIndex);
		...
		public bool Contains (int graphIndex) {
			...
			long2doubleInt(this.value, out var a1, out var a2);

			if (graphIndex >= 31) {
				
				graphIndex -= 31;
				return (a2 & (1 << graphIndex)) != 0;
				
			} else {

				return (a1 & (1 << graphIndex)) != 0;

			}

		}

		public static long Add (int idx, long toMask) {

			long2doubleInt(toMask, out var a1, out var a2);
