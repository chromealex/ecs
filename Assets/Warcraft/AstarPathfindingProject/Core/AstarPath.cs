...
public class AstarPath : VersionedMonoBehaviour {
	...
	/// </summary>
	public void QueueGraphUpdates (double timeSinceStart) {
		if (!graphUpdatesWorkItemAdded) {
			...
			lastGraphUpdate = (float)timeSinceStart;
	...
	IEnumerator DelayedGraphUpdate () {
		...
		QueueGraphUpdates((double)Time.realtimeSinceStartup);
	...
	public void UpdateGraphs(GraphUpdateObject ob) {
		...
		this.UpdateGraphs(ob, 0d);
		
	}

	public void UpdateGraphs (GraphUpdateObject ob, double timeSinceStart) {
		graphUpdates.AddToQueue(ob);

		// If we should limit graph updates, start a coroutine which waits until we should update graphs
		if (batchGraphUpdates && timeSinceStart-lastGraphUpdate < graphUpdateBatchingInterval) {
			...
			QueueGraphUpdates(timeSinceStart);
		}
	}
